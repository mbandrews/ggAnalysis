...
void ggNtuplizer::branchesEE ( TTree* tree, edm::Service<TFileService> &fs ) {
...
void ggNtuplizer::fillEE ( const edm::Event& iEvent, const edm::EventSetup& iSetup ) {
  ...
  for ( EcalRecHitCollection::const_iterator iRHit = EERecHitsH_->begin();
    ...
    vEE_energy_[iz_][idx_] = iRHit->energy();
